package com.payment.krishipay.security;
...
//                kpg.initialize(spec);
        ...
        try {
            ...
            return new SecurityKey(new KeyPair(entry.getCertificate().getPublicKey(), entry.getPrivateKey()));
        } catch (KeyStoreException | NoSuchAlgorithmException | UnrecoverableEntryException e) {
            Timber.e(e);
        }
        return null;
    ...
    static SecurityKey generateSecretKeyPre18(Context context) {
        try {
            ...
            // Fetch Secret Key
            KeyStore.SecretKeyEntry pkEntry = (KeyStore.SecretKeyEntry) androidCAStore.getEntry(KEY_ALIAS, protParam);
