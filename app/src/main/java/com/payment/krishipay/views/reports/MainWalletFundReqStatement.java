...
import com.payment.krishipay.views.reports.adapter.WalletFundReqAdapter;
...
public class MainWalletFundReqStatement extends AppCompatActivity implements RequestResponseLis {
    private static final int REQUEST_COUNT = 0;
    int pastVisiblesItems, visibleItemCount, totalItemCount;
    private List<FundRequestModel> dataList;
    private RecyclerView listView;
    private WalletFundReqAdapter adapter;
    ...
    private void initToolbar() {
        ...
        imgFilter.setOnClickListener(v -> {
            Intent i = new Intent(MainWalletFundReqStatement.this, FilterView.class);
            startActivity(i, ActivityOptions.makeSceneTransitionAnimation(MainWalletFundReqStatement.this).toBundle());
        });
    ...
    private void initReportSlider() {
        adapter = new WalletFundReqAdapter(this, dataList);
    ...
    public void onSuccessRequest(String JSonResponse) {
        ...
        try {
            ...
            if (object.has("data")) {
                ...
                dataList.addAll(AppHandler.parseWalletFundReqTransaction(this, bannerArray));
    ...
    private void initReportPDFData() {
        String[] keys = {"ID", "Type", "Fundbank ID", "Pay Mode", "Amount", "Actual Amount", "Ref Number", "Pay Data", "Status"};
        List<List<InvoiceModel>> modelList = new ArrayList<>();
        for (FundRequestModel model : dataList) {
            ...
            invoiceModels.add(new InvoiceModel("Fundbank", MyUtil.formatWithRupee(this, model.getFundbankId())));
            invoiceModels.add(new InvoiceModel("Mode", MyUtil.formatWithRupee(this, model.getPaymode())));
            invoiceModels.add(new InvoiceModel("Amount", MyUtil.formatWithRupee(this, model.getAmount())));
            invoiceModels.add(new InvoiceModel("Actual Amount", MyUtil.formatWithRupee(this, model.getActualAmount())));
            invoiceModels.add(new InvoiceModel("Ref Number", model.getRefNo()));
            invoiceModels.add(new InvoiceModel("Pay Data", model.getPaydate()));
