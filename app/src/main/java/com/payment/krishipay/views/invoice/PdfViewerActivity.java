package com.payment.krishipay.views.invoice;
...
public class PdfViewerActivity extends PDFViewerActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getSupportActionBar() != null) {
          ...
          //  getSupportActionBar().secolo(getResources().getColor(R.color.colorPrimary));
        }
    }
            ...
            case R.id.menuSharePdf: {
                ...
                if ((Build.VERSION.SDK_INT >= Build.VERSION_CODES.N)) {
                    if (fileToShare.exists())
                        apkURI = FileProvider.getUriForFile(this, getPackageName() + ".files", fileToShare);
                } else {
                    if (fileToShare.exists()) {
                        apkURI = Uri.fromFile(fileToShare);
                    }
                }

                intentShareFile.setDataAndType(apkURI, URLConnection.guessContentTypeFromName(fileToShare.getName()));
                intentShareFile.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
                intentShareFile.putExtra(Intent.EXTRA_STREAM, Uri.parse("file://" + fileToShare.getAbsolutePath()));
